...
export const listActivePRs = async (req: Request, res: Response) => {
  try {
...
export const createRapport = async (req: Request, res: Response) => {
  ...
  if (!patient_id || typeof patient_id !== 'number') return res.status(400).json({ message: 'patient_id requis et doit être un nombre' });
  if (!code_smpau || typeof code_smpau !== 'string') return res.status(400).json({ message: 'code_smpau requis' });
  ...
  try {
    ...
    const ids = intervArray.map((i: any) => Number(i.id)).filter((n: number) => !Number.isNaN(n));
    ...
    if (ids.length > 0) {
      ...
      const foundIds = found.map((p: { id: number }) => p.id);
      const missing = ids.filter((id: number) => !foundIds.includes(id));
      if (missing.length > 0) return res.status(400).json({ message: 'Certains intervenants sont introuvables ou inactifs', missing });
    }

    const rapport = await prisma.$transaction(async (tx: any) => {
      ...
      if (ids.length > 0) {
        const linkCreates = intervArray.map((i: any) => ({
