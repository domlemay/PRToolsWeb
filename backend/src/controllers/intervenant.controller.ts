...
export const listIntervenants = async (req: Request, res: Response) => {
...
export const changePassword = async (req: Request, res: Response) => {
  ...
  if (Number.isNaN(id)) return res.status(400).json({ message: 'ID invalide' });
  if (!new_password || typeof new_password !== 'string') return res.status(400).json({ message: 'Nouveau mot de passe requis' });
...
export const getIntervenant = async (req: Request, res: Response) => {
  if (!isAdmin(req)) return res.status(403).json({ message: 'Accès refusé' });
  ...
  if (!intervenant) return res.status(404).json({ message: 'Intervenant non trouvé' });
