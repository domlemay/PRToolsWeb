...
export const createFullRapport = async (req, res) => {
    ...
    try {
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (Array.isArray(payload.intervenants) && payload.intervenants.length > 0) {
                const linkData = payload.intervenants.map((i) => ({
            ...
            const upsertType = async (table, item) => {
                if (!item)
                    return null;
                if (item.id)
                    return item.id;
                ...
                const created = await tx[table].create({ data });
                return created.id;
            };
            ...
            if (payload.s05) {
                ...
                if (Array.isArray(payload.s05.natures)) {
                    for (const it of payload.s05.natures) {
                        let nid = it.id || null;
                        if (!nid && (it.code || it.libelle))
                            nid = await upsertType('s05_nature_cas_type', it);
                        if (nid)
                            await tx.s05_rapport_nature_cas.create({ data: { rapport_id: createdId, nature_cas_id: nid } });
                    }
                }
                if (payload.s05.info) {
            ...
            if (payload.s06) {
                if (Array.isArray(payload.s06.antecedents)) {
                    for (const it of payload.s06.antecedents) {
                        let aid = it.id || null;
                        if (!aid && (it.code || it.libelle))
                            aid = await upsertType('s06_antecedent_type', it);
                        if (aid)
                            await tx.s06_rapport_antecedent.create({ data: { rapport_id: createdId, antecedent_id: aid } });
                    }
                }
            }
            if (payload.s07) {
                if (payload.s07.info)
                    await tx.s07_maladies_infectieuses_info.create({ data: { rapport_id: createdId, a_maladies_inf: payload.s07.info.a_maladies_inf || false, commentaire: payload.s07.info.commentaire || null } });
                if (Array.isArray(payload.s07.rapports)) {
                    for (const it of payload.s07.rapports) {
                        let mid = it.maladie_infectieuse_type_id || null;
                        if (!mid && (it.code || it.libelle))
                            mid = await upsertType('s07_maladie_infectieuse_type', it);
            ...
            if (payload.s08) {
                if (payload.s08.info)
                    await tx.s08_allergies_info.create({ data: { rapport_id: createdId, a_allergies: payload.s08.info.a_allergies || false, commentaire: payload.s08.info.commentaire || null } });
                if (Array.isArray(payload.s08.rapports)) {
                    for (const it of payload.s08.rapports) {
                        let aid = it.allergie_type_id || null;
                        if (!aid && (it.code || it.libelle))
                            aid = await upsertType('s08_allergie_type', it);
            ...
            if (payload.s09) {
                if (payload.s09.info)
                    await tx.s09_mecanisme_trauma_info.create({ data: { rapport_id: createdId, ...payload.s09.info } });
                if (Array.isArray(payload.s09.rapports)) {
                    for (const it of payload.s09.rapports) {
                        let mid = it.mecanisme_trauma_id || null;
                        if (!mid && (it.code || it.libelle))
                            mid = await upsertType('s09_mecanisme_trauma_type', it);
            ...
            if (payload.s10) {
                if (payload.s10.info)
                    await tx.s10_trauma_info.create({ data: { rapport_id: createdId, ...payload.s10.info } });
                if (Array.isArray(payload.s10.constats)) {
                    for (const c of payload.s10.constats) {
                        let zoneId = c.zone_id || null;
                        if (!zoneId && (c.zone_code || c.zone_libelle))
                            zoneId = await upsertType('s10_trauma_zone_type', { code: c.zone_code, libelle: c.zone_libelle });
                        let lesionId = c.lesion_type_id || null;
                        if (!lesionId && (c.lesion_code || c.lesion_libelle))
                            lesionId = await upsertType('s10_trauma_lesion_type', { code: c.lesion_code, libelle: c.lesion_libelle });
                        if (zoneId && lesionId)
                            await tx.s10_trauma_constat.create({ data: { rapport_id: createdId, zone_id: zoneId, lesion_type_id: lesionId, commentaire: c.commentaire || null } });
                    }
                }
            }
            if (payload.s11)
                await tx.s11_support_clinique_info.create({ data: { rapport_id: createdId, ...payload.s11 } });
            if (Array.isArray(payload.s12)) {
                for (const sv of payload.s12)
                    await tx.s12_signe_vital.create({ data: { rapport_id: createdId, heure_prise: new Date(sv.heure_prise), respiration_min: sv.respiration_min || null, pouls_min: sv.pouls_min || null, etat_conscience_avpu: sv.etat_conscience_avpu || null, glycemie: sv.glycemie || null, spo2_pct: sv.spo2_pct || null, o2_pct: sv.o2_pct || null, created_at: new Date() } });
            }
            if (payload.s13) {
                if (payload.s13.info)
                    await tx.s13_info.create({ data: { rapport_id: createdId, non_intervention: payload.s13.info.non_intervention || null, commentaire: payload.s13.info.commentaire || null } });
                if (payload.s13.arret_cardiaque)
                    await tx.s13_arret_cardiaque.create({ data: { rapport_id: createdId, ...payload.s13.arret_cardiaque } });
                if (payload.s13.voies_aeriennes)
                    await tx.s13_voies_aeriennes.create({ data: { rapport_id: createdId, ...payload.s13.voies_aeriennes } });
                if (payload.s13.medicaments)
                    await tx.s13_medicaments.create({ data: { rapport_id: createdId, ...payload.s13.medicaments } });
            }
            if (payload.s14)
                await tx.s14_entreprise_ambulanciere.create({ data: { rapport_id: createdId, ...payload.s14 } });
            if (payload.s15)
                await tx.s15_autres_renseignements.create({ data: { rapport_id: createdId, ...payload.s15 } });
            return rapport;
        });
...
//# sourceMappingURL=rapport.full.controller.js.map